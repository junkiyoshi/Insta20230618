...
Actor::Actor(vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list) {
	...
	this->color.setHsb(ofRandom(4) * 64, 150, 255);
}

//--------------------------------------------------------------
void Actor::update(const int& frame_span, vector<glm::vec3>& location_list, vector<vector<int>>& next_index_list, vector<int>& destination_list, uint64_t frame_number) {
...
void ofApp::setup() {
	...
	auto span = 20;
	for (int x = -320; x <= 320; x += span) {
		
		for (int y = -320; y <= 320; y += span) {
...
void ofApp::update() {
	...
	for (auto& actor : this->actor_list) {
		...
		actor->update(frame_span, this->location_list, this->next_index_list, this->destination_list, frame_number);
	}
